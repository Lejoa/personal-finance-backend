...
class ChatService 
{
    ...
    /**
     * Processes a user message and returns the LLM resposne
     */
    ...
    private function getOrCreateConversation(ChatRequestDTO $dto, User $user): ChatConversation
    {
        if ($dto->conversationId !== null) {
            $conversation = 
                $this->conversationRepository->findOneByIdAndUser(
                    $dto->conversationId, 
                    $user
                );
    ...
    private function callLlmService(LlmChatRequestDTO $request): array
    {
        $response = $this->httpClient->request(
            'POST', 
            $this->llmServiceUrl . '/llm/chat', 
            ['json' => $request->toArray(),'timeout' => 30]
        );
